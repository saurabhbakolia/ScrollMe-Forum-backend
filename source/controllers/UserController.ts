...
export const loginOne = async (req: Request, res: Response) => {
    try {
        ...
        res.cookie("accessToken", accessToken, { httpOnly: true, maxAge: 1 * 60 * 1000 }); // 10 min
        res.cookie("refreshToken", refreshToken, { httpOnly: true, maxAge: 2 * 24 * 60 * 60 * 1000 }); // 1 day
        res.status(200).json({
            "status": "success",
            "code": 200,
